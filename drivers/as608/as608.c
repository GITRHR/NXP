...
uint8_t  USART0_RX_BUF[USART2_MAX_RECV_LEN]; 		//接收缓冲,最大USART2_MAX_RECV_LEN字节
uint16_t USART0_RX_STA=0;   						//接收数据状态
...
static uint8_t *JudgeStr(uint16_t waittime)
{
	...
	{
		delay_ms(15);
		if(USART0_RX_STA&0X8000)//接收到一次数据
		{
			USART0_RX_STA=0;
			data=strstr((const char*)USART0_RX_BUF,(const char*)str);
			if(data)
				return (uint8_t*)data;	
		}
	}
